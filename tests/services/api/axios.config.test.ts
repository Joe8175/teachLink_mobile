...
describe('axios.config - Rate Limit Handling (Issue #141)', () => {
    ...
    describe('429 Rate Limit - Retry Logic', () => {
        ...
        it('should retry 429 once with 1000ms delay on first attempt', async () => {
            ...
            axiosMockAdapter = jest.spyOn(axios.defaults, 'adapter' as any).mockImplementation(
                async (config: any) => {
                    if ((config as any)._retryCount === undefined) {
            ...
            // Make mock request and expect first 429 retry
            // Note: In real scenario, interceptor handles this automatically
    ...
    describe('User Feedback - Logger Integration', () => {
        ...
        it('should call logger.warn for each retry attempt', () => {
            ...
            logger.warn('[RATE_LIMIT] Retry 1/5 in 1s: GET /courses');
        ...
        it('should call logger.error when max retries exceeded', () => {
            logger.error('[RATE_LIMIT] Max retries exceeded (5 attempts) for GET /courses');
